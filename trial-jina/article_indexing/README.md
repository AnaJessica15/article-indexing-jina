...
```bash
docker build -t jinaai/hub.app.article_indexing:0.1.0 .
```

To mount local directory and run:
```bash
docker run -v "$(pwd)/j:/workspace" jinaai/hub.app.article_indexing:0.1.0
``` 

To query
```bash
docker run -p 65482:65482 -e "JINA_PORT=65481" jinaai/hub.app.article_indexing:0.1.0 search
```
...
Copyright (c) 2020 Ana. All rights reserved.
